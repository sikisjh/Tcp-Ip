...
void receiveMessages(void* sock) {
    ...
    while (1) {
        ...
        if (bytesReceived > 0) {
            ...
            snprintf(combinedMessage, sizeof(combinedMessage), "%s", recvBuf);
...
int main() {
    ...
    while (1) {
        if (numClients < MAX_CLIENTS) {
            ...
            HANDLE clientThread = CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)receiveMessages, (LPVOID)&clientSocket, 0, NULL);
